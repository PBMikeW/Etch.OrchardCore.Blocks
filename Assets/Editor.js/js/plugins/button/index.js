...
class Button {
  ...
  createInput(id, type, inputLabel, inputPlaceholder) {
    ...
    createSelect(id, inputLabel) {
        ...
        let colorConfigs = this.config.color;
        
        for(const [name, cssClass] of Object.entries(colorConfigs)) {
        ...
        if(this.data["colorStyle"]) select.value = this.data["colorStyle"];
    ...
    const urlInput = this.createInput('url', 'text', "Url", "Placeholder")
    const labelInput = this.createInput('label', 'text', "Text", "Placeholder")
    const colorSelector = this.createSelect('colorStyle', "Color");
      
    title.innerText = this.api.i18n.t('Button')
    title.classList.add(this.CSS.title)

    wrapper.classList.add("row","pt-2", "pb-3", "mx-1", this.CSS.container);    
    title.classList.add("col-12");
    urlInput.classList.add("col-6");
    labelInput.classList.add("col-6");
    colorSelector.classList.add("col-6");
	...
	save(block){
        ...
        const colorStyle = block.querySelector('#colorStyle');
        return {
          url: url.value,
          label: label.value,
